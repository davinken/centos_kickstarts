############################################################################ 
# +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
# |i|c|o|s|7|p|a|r|t|i|t|i|o|n|i|n|g|_|s|m|a|l|l|v|m|.|k|s|
# +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
# icos7partitioning_smallvm.ks
...
# 08sep2016/davidr - Customized for small KVM VMs. Reduced size, and disk
#                    name is vda (instead of sda).

# "SMALL DISK" - intended for max. 80 GB disks (single purpose, not file-serving
# oriented machines).
...
logvol /  --fstype="ext4" --size=40000 --label="parasol_root" --name=root --vgname=centos_parasol
